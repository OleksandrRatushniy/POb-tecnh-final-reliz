...
class Program // основний клас
{
    ...
    static void Main(string[] args) // основна точка входу в програму, Коли запускаєм програму, виконання починається з методу Main.
    {
        ...
        EquipmentList = new List<Equipment>();// Цей рядок ініціалізує глобальний список EquipmentList як новий список об’єктів Equipment

        // Перевірка наявності файлу і зчитування даних з файлу
        if (File.Exists(@"C://Temp//pobutova technica.txt"))
        {
            string jsonText = File.ReadAllText(@"C://Temp//pobutova technica.txt");  //Якщо файл існує, цей рядок читає весь вміст файлу як рядок і зберігає його в змінній jsonText.
            List<Equipment> equipmentList = JsonSerializer.Deserialize<List<Equipment>>(jsonText);// Цей рядок десеріалізує JSON текст з файлу назад в список об’єктів Equipment.
            // Додавання зчитаних даних до глобального списку обладнання
            EquipmentList.AddRange(equipmentList);//  Цей рядок додає всі об’єкти з equipmentList до глобального списку EquipmentList.

            // Упорядкування порядкового номера з 0 лічильник + при виведені кожен раз обновлює порядковий номер
            for (int i = 0; i < EquipmentList.Count; i++)// Цей рядок починає цикл for, який проходить через кожен елемент в EquipmentList.
            {
                EquipmentList[i].OrdinalNumber = i + 1;// порядковий номер встановлюється на 1 кожен наступний +1
            }
    ...
    //Equipmentpublic означає, що цей клас доступний для всього коду в проекті. Клас - це схема або “синтаксичний цукор” для створення об’єктів, які представляють певну сутніст
